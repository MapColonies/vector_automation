...
from locust import HttpUser, constant, tag, task, events
from locust.runners import MasterRunner


class QueryService(HttpUser):
    constant(1)
    ...
    def __init__(self, *args, **kwargs):
        ...
        query_request = f"?service=wfs&version=2.0.0&request=GetFeature&typeNames={featureType}&outputFormat" \
                        f"=json&count={numbers_of_req}&sortBy={by_date} "
        with self.client.get(query_request, catch_response=True, verify=False, headers=headers_auth,
                             name="GetFeaturePostOnStart") as resp:
        ...
        query_request = f"?service=wfs&version={version}&request=GetCapabilities"
        with self.client.get(query_request, catch_response=True, verify=False, headers=headers_auth,
    ...
    def test_get_capabilities(self):
        query_request = f"?service=wfs&version={version}&request=GetCapabilities"
        with self.client.get(query_request, catch_response=True, verify=False, headers=headers_auth,
                             name="GetCapabilities") as response:
            if 200 == response.status_code:
                response.success()
            else:
                response.failure(str(response.status_code) + " Query has sent: " + query_request)
    ...
    def test_describe_feature_type_get(self):
        query_request = f"?service=wfs&version={version}&request=DescribeFeatureType&typeNames={featureType} "
        with self.client.get(query_request, catch_response=True, verify=False, headers=headers_auth,
    ...
    def test_get_feature_geometry(self):
        bbox_cor_list = [random.uniform(bbox_x[0], bbox_x[-1]),
                         ...
                         random.uniform(bbox_y[0], bbox_y[-1])]
        query_request = f"?service=wfs&version={version}&request=GetFeature&typeNames={featureType}" \
    ...
    def test_post_feature_geometry(self):
        query_request = f"?service=wfs&version={version}&request=GetFeature&typeNames={featureType}&count={str(random.randint(1, 9))}" \
                        f"&type={str(self.choice_params())}"
        with self.client.post(query_request, catch_response=True, verify=False, headers=headers_auth,
                              name="GeometryFieldPost") as response:
            if 200 == response.status_code:
                response.success()
            else:
                response.failure(str(response.status_code) + " Query has sent: " + query_request)
    ...
    def test_get_feature_date(self):
        ...
        query_request = f"?service=wfs&version={version}&request=GetFeature&typeNames={featureType}&{by_date}={ran_date}"
    ...
    def test_get_feature_grid(self):
        ...
        query_request = f"?service=wfs&version={version}&request=GetFeature&typeNames={featureType}&featureId='{geo_id[gfid]}'"
        with self.client.post(query_request, catch_response=True, verify=False, headers=headers_auth,
                              name="GetFeatureGfid") as response:
            if 200 == response.status_code:
